...
export const filterArrays = (userArray: Users[], armyArray: Armies[], setFilteredArmyArray: SetArmyArray,setFilteredUserArray: SetUserArray, formik: Formik ) => {
    ...
    const secondFilteredResponse = filteredResponse?.filter((user: Player) => {
      const userOneBool = formik.values.playerOne.find((player) => player.id === user.id);
      const userTwoBool = formik.values.playerTwo.find((player) => player.id === user.id);
